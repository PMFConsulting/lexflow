...
/**
 * Associa um utilizador sem gestor à equipa do próprio gestor que executa a ação.
 * Restrito ao papel 'gestor'.
 */
export async function associarUtilizadorEquipa(
  utilizadorId: string,
): Promise<{ ok: true } | { ok: false; erro: string }> {
  const s = await import("@/lib/sessao").then(m => m.exigirEquipaDaSociedade());
  const { eu } = s;

  if (eu.papel !== "gestor") {
    return { ok: false, erro: "Apenas um gestor pode adicionar utilizadores à sua equipa." };
  }
  ...
  if (alvo.papel !== "utilizador") {
    return { ok: false, erro: "Apenas contas com papel 'utilizador' podem ser associadas." };
  }

  if (!alvo.ativo) {
    return { ok: false, erro: "Não é possível associar um utilizador inativo." };
  }

  if (alvo.gestorId === eu.id) {
    return { ok: true };
  }

  if (alvo.gestorId !== null) {
    return { ok: false, erro: "Este utilizador já tem um gestor associado." };
  }

  await db()
    .update(utilizador)
    .set({ gestorId: eu.id, atualizadoEm: new Date() })
    .where(eq(utilizador.id, utilizadorId));
...
/**
 * Remove um utilizador da equipa do próprio gestor.
 */
export async function removerUtilizadorEquipa(
  utilizadorId: string,
): Promise<{ ok: true } | { ok: false; erro: string }> {
  const s = await import("@/lib/sessao").then(m => m.exigirEquipaDaSociedade());
  const { eu } = s;
  ...
  if (eu.papel !== "gestor") {
    return { ok: false, erro: "Apenas um gestor pode remover utilizadores da sua equipa." };
  }
  ...
  if (alvo.gestorId !== eu.id) {
    return { ok: false, erro: "Este utilizador não pertence à sua equipa." };
  }

  await db()
    .update(utilizador)
    .set({ gestorId: null, atualizadoEm: new Date() })
    .where(eq(utilizador.id, utilizadorId));
